...
BEGIN
    ...
    IF input_category IS NOT NULL THEN
        SET @category_search = CONCAT('%', input_category, '%');
    ELSE
        SET @category_search = NULL;
    END IF;
    ...
    IF prefix_brand_id IS NOT NULL THEN
        ...
        WHERE
            ...
            -- 2. Added the missing Category Check here
            AND (@category_search IS NULL OR p.product_type LIKE @category_search)
        GROUP BY
    ...
    ELSE
        ...
        FROM
            ...
            LEFT JOIN sizes s ON ps.size_id = s.size_id
        WHERE
            (@category_search IS NULL OR p.product_type LIKE @category_search)
