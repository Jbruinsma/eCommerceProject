      ...
      <div class="wizard-card">
        ...
        <div v-else-if="submissionResult" class="submission-result-screen">
          <div v-if="submissionResult.success">
            ...
            <button @click="router.push('/my-listings')" class="btn btn-primary">
              View My Listings
            </button>
          </div>
          <div v-else>
            ...
            <p>
              {{ submissionResult.message || "We couldn't create your listing. Please try again." }}
            </p>
            ...
            <div class="form-group" v-if="listingData.item_condition">
              ...
              <div v-if="availableSizesForListing.length > 0" class="radio-group size-group">
                <button
                  ...
                  :key="size.size_id"
                  class="radio-btn"
                  :class="{ selected: listingData.size_id === size.size_id }"
...
// MODIFIED: Now stores bidId along with bidAmount
        ...
        for (const condition in sizeData.bids) {
          ...
          if (bidInfo && bidInfo.bidAmount > 0) {
            ...
            formattedBids[key] = {
              ...
              bid_id: bidInfo.bidId // Store the bid ID
            };
          }
        }
      }
...
// MODIFIED: Function now handles 'sale' type by adding the listing_id
async function submitListing() {
  ...
  try {
    ...
    if (listingType === 'ask') {
      listingResponse = await postToAPI(`/listings/${authStore.uuid}/create`, payload);
      ...
      // // Assuming the same endpoint is used for creating a sale from a bid
      // listingResponse = await postToAPI(`/listings/${authStore.uuid}/create`, payload);
    }
