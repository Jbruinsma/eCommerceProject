      ...
      <div class="listing-grid">
        <div class="product-summary">
          ...
          <div class="product-details">
            <h3>{{ listing.product.brand.brand_name }}</h3>
        ...
        <div class="listing-sidebar">
          ...
          <div class="info-section">
            ...
            <ul>
              <li><span>Asking Price</span><span>{{ formatCurrency(listing.price) }}</span></li>
              <li><span>Transaction Fee (9.5%)</span><span class="negative">-{{ formatCurrency(transactionFee) }}</span></li>
              <li class="total"><span>Seller Payout</span><span>{{ formatCurrency(sellerPayout) }}</span></li>
            </ul>
          </div>
          ...
          <div class="info-section">
            ...
            <ul>
              <li><span>Listed On</span><span>{{ formatDate(listing.created_at) }}</span></li>
              <li v-if="listing.status === 'sold'"><span>Sold On</span><span>{{ formatDate(listing.sold_at) }}</span></li>
            </ul>
          </div>
        </div>
      </div>
...
// Mock data simulating a detailed query for a single listing.
const listing = ref({
  listing_id: 501,
  price: '310.00',
  item_condition: 'new',
  status: 'active', // Can be 'active', 'sold', or 'canceled'
  created_at: '2025-10-15T11:30:00Z',
  sold_at: '2025-10-20T11:30:00Z', // Only relevant if status is 'sold'
  product: {
    name: 'Retro Dunk "Obsidian"',
    image_url: 'https://placehold.co/600x600/1a1a1a/ffffff?text=Sneaker',
    brand: { brand_name: 'Athletic Co.' }
  },
  size: { size_value: '10.5' }
});

// --- To test a "sold" state, you can swap the mock data ---
/*
const listing = ref({
  listing_id: 503,
  price: '120.00',
  ...
  sold_at: '2025-10-02T10:00:00Z',
  product: {
    name: 'Streetwise Hoodie "Carbon"',
    image_url: 'https://placehold.co/600x600/1a1a1a/ffffff?text=Hoodie',
...
const transactionFee = computed(() => {
  return parseFloat(listing.value.price) * 0.095; // Assuming a 9.5% transaction fee
});

const sellerPayout = computed(() => {
  return parseFloat(listing.value.price) - transactionFee.value;
});
