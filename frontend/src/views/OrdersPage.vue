...
onMounted(async () => {
  ...
  try {
    ...
    ordersList.value = []; // Ensure ordersList is an empty array on error
  } finally {
...
/**
 * Normalizes the API response to always return an array of orders,
 * handling cases where the API might return a single object or nested data.
 * @param {any} res The raw response from the API.
...
const normalizeOrders = (res) => {
  ...
  if (typeof res === 'object') return [res]; // Fallback for single object response
  return [];
};

/**
 * Formats a currency amount into a USD string (e.g., 123.45 => $123.45).
...
/**
 * Formats a date string into a more readable format (e.g., "October 22, 2025").
...
const formatDate = (dateString) => {
  ...
  if (Number.isNaN(d.getTime())) return dateString; // Return original if invalid
 ...
 * @param {string} s The status string (e.g., "pending").
 * @returns {string} The formatted status (e.g., "Pending").
 */
